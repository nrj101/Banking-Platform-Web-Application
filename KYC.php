...
<head>
	...
	<style>
			...
			#form-container{padding: 10px 0px 80px 0px;}
...
<div id="page" class="w3-container">
	...
	<div id="header" class="w3-container w3-col m12 l12 w3-center">
		<div id="logo">
			...
			<h1 class="heading" ><!--style="border:2px solid #f90e65;"--> <abbr title="Father's Bank Of India">:: FBI</abbr> -> KYC Page ::</h1>
		</div>		
		<!--Navigation Bar-->
...
?>	
	...
	<div id="content-form" class="w3-container w3-col m12 l12 w3-center simple-form">			
			<form id="form-container" enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF']?>" method="post">				
				...
				<label>Aadhaar No. : </label><input type="text" name="Aadhaar" id="button" maxLength="12"  pattern="[0-9]{12}" title="Required Valid 12-digit AADHAAR No." id="button" required placeholder="Enter your AADHAAR No."><br><br>
				<label>Current Address : </label><textarea name="addr" id="button" maxLength="70" rows="2" cols="45" id="button" required placeholder="Enter your address in full, with PIN code..."></textarea><br><br>
				...
				<input type="submit" name="Submit" id="butt" value="Upload" />
			</form>
...
function KYC() 
	{
		...
		if($result)
		{
			...
			$gender=$info['Gender'];
			if($gender=="Male")
				$fname="Mr. ".$_POST['fname'];
			else if($gender=="Female")
				$fname="Mrs. ".$_POST['fname'];
			else
				$fname=$_POST['fname'];
			$lname=$_POST['lname'];		
			...
			{
				//$query="UPDATE CUSTOMERS SET KYC=1 WHERE AC_NO='$ac_no';";
				$query="INSERT INTO KYC VALUES('$ac_no','$name','$dob','$addr','$aadhaar','$pUpload$pNewName','$sUpload$sNewName','$idUpload$idNewName');";
				if(mysqli_query($con,$query))
				{
					//  display  success  message
					echo  "<br>Your KYC form has been received.Your Account will be reinstated shortly after approval.<br>";
				}
				else
					echo "<br>Error Submitting KYC form!! Please Try Again ..<br>";			
			}
		}
		else
			echo "<br>Error Submitting KYC form!! Please Check the submitted details and try again ..<br>";
