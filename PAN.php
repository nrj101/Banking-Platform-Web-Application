...
<div id="page" class="w3-container">
	...
	<div id="header" class="w3-container w3-col m12 l12 w3-center">
		<div id="logo">
			...
			<h1 class="heading" ><!--style="border:2px solid #f90e65;"--> <abbr title="Father's Bank Of India">:: FBI</abbr> -> Link PAN ::</h1>
		</div>	
		<!--Navigation Bar-->
...
?>	
	...
	<div id="content-form" class="w3-container w3-col m12 l12 w3-center simple-form">
			<form id="form-container" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>" method="post">
				<h2 class="form-name w3-bar">&nbsp;&nbsp;&nbsp;PAN Linking Form&nbsp;&nbsp;&nbsp;</h2><br>
				...
				<label>PAN : </label><input type="text" name="PAN"  maxLength="10" onblur="this.value=this.value.toUpperCase()"  pattern="[A-Z]{5}[0-9]{4}[A-Z]" title="Required Valid PAN" id="button" required placeholder="Enter PAN "><br><br>
...
function PAN()
{
    ...
    if(!empty($_POST['ac_no']))
    {
        ...
        else
        {
            ...
            if($row['PAN']!=NULL)
            {
                echo "<h2>PAN is already seeded.</h2>If you want to change/update PAN details, please fill in the KYC form.";
            }
            else
            {
                 $query="UPDATE CUSTOMERS SET PAN='$PAN' WHERE AC_NO='$ac_no'";
                 $data=mysqli_query($con,$query) or die(mysqli_error($con));
                 if($data)
                 {
                     echo "PAN entered successfully";
                 }
                else
                {
