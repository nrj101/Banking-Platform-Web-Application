...
?>
	...
	<form id="form-container" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" >
			...
			<label> Transfer From : </label><input type="number" name="AC_NO" id="button" required placeholder="From Account No." ><br><br>
			<label> Transfer To : </label><input type="number" name="AC" id="button" required placeholder="To Account No." ><br><br>
			...
			<label> Payment Mode : </label><input type="text" name="Details" id="button" maxLength="20" onblur="this.value=this.value.toUpperCase()" placeholder="Cheque No./DD No./Debit Card" pattern="[A-Z- 0-9]{4,20}" title="For Cheque: CHQ-< Cheque No. >  For Debit Card: Debit Card" required><br><br>
			...
			<input type="submit" name="Transfer" value="Transfer" id="butt"><br><br><br><br>
	</form>
<?php
}
else
...
<?php
	function Transfer()
	{
		...
		$ac_no=$_POST['AC_NO'];
		$Amount=$_POST['Amount'];
		$Details=" - ".$_POST['Details']." Transfer to ".$_POST['AC'];
		...
		else
		{
			...
			{
				echo "<div align=\"center\"><br><h3>Insufficient Balance !!  Transaction Aborted.</h3>";
