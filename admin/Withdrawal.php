...
<div id="page" class="w3-container">
	...
	<div id="header" class="w3-container w3-col m12 l12 w3-center">
		...
		<div id="navigation" class="w3-bar">
			...
			<a href="Withdrawal.php" class="w3-bar-item w3-button w3-round-xlarge w3-padding-large w3-red w3-hover-lime">Withdraw</a>
...
?>
	...
	<form id="form-container" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" >
		<h2 class="form-name w3-bar">&nbsp;&nbsp;&nbsp;Money Withdrawal Form&nbsp;&nbsp;&nbsp;</h2><br>
			<label> Withdraw From : </label><input type="text" name="AC_NO" id="button" maxLength="7" pattern="[1][0][1][0-9]{4}" title="Required Valid 7 digit A/C No." placeholder="Account No." required><br><br>
			...
			<label> Transaction Mode : </label><input type="text" name="Details" id="button" required pattern="[A-Z- 0-9]{4,20}" title="For Cheque/DD: CHQ/DD-< Cheque/DD No. >" onblur="this.value=this.value.toUpperCase()" placeholder="Self / Cheque No." ><br><br>
			...
			<label> Payee : </label><input type="text" name="Payee" id="button" required pattern="[A-Z ]{3,20}" title="Required NAME 3 character or longer" onblur="this.value=this.value.toUpperCase()" placeholder="Name of Recipient" ><br><br>
			<input type="submit" name="Withdraw" value="Withdraw" id="butt" ><br><br><br><br>
	</form>
<?php
}
else
...
<?php
	function Withdraw()
	{
		...
		$ac_no=$_POST['AC_NO'];
		$Amount=$_POST['Amount'];
		$Details=" - ".$_POST['Details']." for ".$_POST['Payee'];
		...
		else
		{
			...
			if($Amount>$info['Balance'])
				echo "<br><h3>Insufficient Balance !!  Transaction Aborted.</h3>";
			else
			{
			
				$query="INSERT INTO `$ac_no` VALUES (NULL,'$Details', NOW(),'0.00','$Amount','$Balance')";
				$result=mysqli_query($con,$query) or die(mysqli_error($con));
				if($result)
				{
					echo "<div align=\"center\"><br><h3>Transaction Successful</h3>";
					echo "<br>Rs. $Amount Debited from A/c : $ac_no.  Available Balance is Rs.$Balance</div>";				
				}
			}
